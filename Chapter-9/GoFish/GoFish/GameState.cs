...
public class GameState
{
	...
	public string PlayRound(Player player, Player playerToAsk, Values valueToAskFor, Deck stock)
	{
		...
		var matchingCards = playerToAsk.DoYouHaveAny(valueToAskFor, stock);
		if (matchingCards.Count() > 0)
		{
			player.AddCardsAndPullOutBooks(matchingCards);
			message += $"{playerToAsk.Name} has {matchingCards.Count()} {valueToAskFor} card{Player.S(matchingCards.Count())}";
		}
